...
public class ConnectedComponentsTest {
    ...
    public void ex1() {
        ...
        new ConnectedComponents().find(vs, vs.length);
    }
    ...
    public void ex4WithView() {
        SATAlgorithmL.Literal[] vs = createVertexArray(11, 0,1, 0,3, 1,2, 1,4, 2,0, 2,6, 3,2, 4,5, 4,6, 5,6, 5,7, 5,8, 5,9, 6,4, 7,9, 8,9, 9,8);
        SATAlgorithmL.Literal[] view = new SATAlgorithmL.Literal[]{vs[2], vs[3], vs[4], vs[5]};
        new ConnectedComponents().find(view, view.length);
        //noinspection unchecked
        assertThat(Arrays.stream(view).collect(Collectors.toMap(x -> x.id, x -> x.parent.id)),
